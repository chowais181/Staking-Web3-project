async function main() {
  ...
  console.log("staking Token => Address: ", token.address);
  ...
  //  {
  console.log("Approving Pool =>", token_instance.address);
  await token_instance
    .connect(deployer)
    .approve(staking_instance.address, ethers.utils.parseEther("10000"));
  for (let i = 0; i < 3; i++) {
    await staking_instance.connect(deployer).create_pool(i + 1, 0);
  }
  for (let i = 3; i < 6; i++) {
    await staking_instance.connect(deployer).create_pool(i + 1, 1);
  }
  value = await token_instance.balanceOf(deployer.address);
  console.log("Before Staking: ", value);
  await staking_instance
    .connect(deployer)
    .staking(1, ethers.utils.parseEther("1"));
  value = await token_instance.balanceOf(deployer.address);
